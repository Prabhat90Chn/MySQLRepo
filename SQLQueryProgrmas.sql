...
Begin
Insert into StudentInfo(StudentID,StudentName,ParentName,PhoneNumber,AddressOfStudent,City,Country)
Values (@StudentID,@StudentName,@ParentName,@PhoneNumber,@Adress,@City,@Country)
END
select * from StudentInfo;
...
AS
Begin
update studentinfo set city = @City where StudentID = @StudentID
End
...
drop table ProductInvetory;
Alter table ProductInventory add QuantityAvailable int;

insert into ProductInventory values('IPhone',101);

insert into ProductInventory values('IPad',134);

select * from ProductInventory;


create procedure sp_addProductInventory
@ProductName varchar(50),
  int,
@id int out
as
begin
insert into ProductInventory values ( @ProductName, @QuantityAvailable );
select @id = SCOPE_IDENTITY()
end

Declare @id int
Execute sp_addProductInventory 'Itag','320',@id out
Print 'id is ='+ cast(@id as nvarchar(4))

use STUDENT1;

Select id,ProductName,QuantityAvailable from ProductInventory
...
insert into ProductCategory values('Sports');
insert into ProductCategory values('Kitchen');
select * from ProductCategory; Select * from ProductInventory;



Create Procedure sp_MultiTable
as
begin
Select * from ProductInventory;
...
-- Procedure to using tow tables
CREATE PROCEDURE GetCustomersWithOrdersInDateRange(
    IN startDate DATE,
    IN endDate DATE
)
BEGIN
...
CALL GetCustomersWithOrdersInDateRange('2024-01-01', '2024-04-01');


-- procedure using sub query
...
BEGIN
    ...
    SELECT CONCAT('Total price of all products: ', total) AS Result;
END//
...
CREATE FUNCTION AddTwoNumbers(x INT, y INT)
RETURNS INT
BEGIN
    DECLARE result INT;
    SET result = x + y;
...
SELECT AddTwoNumbers(3, 5);

CREATE FUNCTION GetFullName(customer_id INT)
RETURNS VARCHAR(100)
BEGIN
